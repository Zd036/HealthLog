...
import com.example.heartcare.ui.home.HomeUiState
...
fun IntakeOutputCard(
    ...
    BigCard {
        ...
        if (!state.hasIntakeOutput) {
            ...
            Text("总摄入：${state.totalIntake.toInt()} ml",
                style = MaterialTheme.typography.titleLarge.copy(color = PrimaryBlue))
            Spacer(Modifier.height(6.dp))
            Text("总排出：${state.totalOutput.toInt()} ml" +
            ...
            Text("差值：$sign$diff ml",
                style = MaterialTheme.typography.titleLarge.copy(color = color))
        }
        Spacer(Modifier.height(16.dp))
        Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
            PrimaryBigButton("记摄入", onClick = onIntake, modifier = Modifier.weight(1f))
            PrimaryBigButton("记排出", onClick = onOutput, modifier = Modifier.weight(1f),
                color = PrimaryBlue)
        }
    }
}
